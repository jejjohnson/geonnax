...
make typecheck            # Type check: ty check src/geonnax
...
uv run --group lint ruff check .              # Lint — ENTIRE repo, not just src/geonnax/
uv run --group lint ruff format --check .     # Format — ENTIRE repo
uv run --group typecheck ty check src/geonnax  # Typecheck — package only
```

**Critical**: Always lint/format with `.` (repo root), not `src/geonnax/`. CI runs `ruff check .` which includes `tests/` and `scripts/`.

## Architecture

### Package structure

All implementation lives in `src/geonnax/`. The public API is re-exported through `src/geonnax/__init__.py`.
